...
<div class="space-y-4" x-data="{ uploadOpen: {{ $errors->any() ? 'true' : 'false' }}, rejectTarget: null }">
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="border-b border-gray-100 text-gray-500">
                        ...
                        <th scope="col" class="py-2 pr-4">Proof</th>
                    ...
                    @forelse ($payments as $payment)
                        <tr class="border-b border-gray-50">
                            ...
                            <td class="py-2 pr-4">
                                ...
                                @if ($canVerify)
                                    <dialog aria-labelledby="payments-dialog-1-{{ $payment->id }}"
                                            x-effect="(rejectTarget === {{ $payment->id }}) ? ($el.open || $el.showModal()) : ($el.open && $el.close())"
                                            x-on:close="rejectTarget = null" @click.self="$el.close()"
                                            class="w-[calc(100%-2rem)] max-w-md rounded-xl bg-transparent p-0 shadow-lg">
                                        <div class="rounded-xl bg-white p-6">
                                            <h2 id="payments-dialog-1-{{ $payment->id }}" class="mb-3 text-lg font-semibold text-gray-900">Reject Payment</h2>
    ...
    @if ($isTenant)
        <dialog aria-labelledby="payments-dialog-2"
                x-effect="(uploadOpen) ? ($el.open || $el.showModal()) : ($el.open && $el.close())"
                x-on:close="uploadOpen = false" @click.self="$el.close()"
                class="w-[calc(100%-2rem)] max-w-lg rounded-xl bg-transparent p-0 shadow-lg">
            <div class="rounded-xl bg-white p-6">
                <h2 id="payments-dialog-2" class="mb-4 text-lg font-semibold text-gray-900">Upload Payment Proof</h2>
                ...
                <form id="payment-form" method="POST" action="{{ route('tenant.payments.store') }}" enctype="multipart/form-data" class="grid grid-cols-1 gap-3"
                    ...
                    <div>
                        <label class="block">
                            <span class="mb-1 block text-sm font-medium text-gray-700">Bill</span>
                    ...
                    <div>
                        <label class="block">
                            <span class="mb-1 block text-sm font-medium text-gray-700">Proof of Payment</span>
